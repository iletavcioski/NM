...
function [U, k] = jacobi(Uk, tol, maxk)
  U = Uk;
  ...
  rad = linspace(1, 2, n);%dolzino radij
  while k < maxk && napaka > tol
    ...
    for i = 2 : n - 1
      for j = 1 : m
        ...
        U(i, j) = ((Uk(i + 1, j) * (h_k^2 *(rad(i)*h_r + rad(i)^2))) + (Uk(i - 1, j) *(rad(i)^2*h_k^2))
        + ((Uk(i,jp) + Uk(i,jm))*(h_r*h_r)))/((rad(i) * h_r * h_k^2) + (2*rad(i)^2*h_k^2) + (2*h_r^2));
